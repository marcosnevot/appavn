document.addEventListener('DOMContentLoaded', function () {
    ...
    function loadFilteredTasks(page = 1, sortKey = 'fecha_planificacion', sortDirection = 'asc') {
        ...
        const filterData = {
            ...
            asunto: document.getElementById('filter-asunto-ids').value || 'CITA,LLAMADA TELEFONICA', 
        ...
        if (filterEntries.length === 0) {
            ...
            filterEntries.forEach(([key, value]) => {
                ...
                if (key === 'cliente') {
                    ...
                    p.textContent = `Mostrando Citas/Llamadas De: ${userNames}`;
                } else {
    ...
    function filtrarTareasPorPlanificacion(fecha, sortKey = 'fecha_planificacion', sortDirection = 'asc') {
        ...
        const filterData = {
            cliente: document.getElementById('filter-cliente-ids').value || '', // Usar los IDs de clientes seleccionados
            asunto: document.getElementById('filter-asunto-ids').value || 'CITA, LLAMADA TELEFONICA',  // Usar los IDs/nombres de asuntos seleccionados
            tipo: document.getElementById('filter-tipo-ids').value || '',
            subtipo: document.getElementById('filter-subtipo-ids').value || '', // Usar el campo oculto con los valores seleccionados
    ...
    function showAutocompleteField(header, autocompleteContainer, dataList, field) {
        ...
        const fieldMapping = {
            cliente: { hidden: '#filter-cliente-ids', visible: '#filter-cliente-input' },
            asunto: { hidden: '#filter-asunto-ids', visible: '#filter-asunto-input' },
            tipo: { hidden: '#filter-tipo-ids', visible: '#filter-tipo-input' }
        };
            ...
            if (!query) {
                ...
                Object.keys(fieldMapping).forEach(field => {
                    if (input.id.includes(field)) {
                        const { hidden, visible } = fieldMapping[field];
        ...
        data.forEach(item => {
            if (item && item[property]) {
                ...
                li.addEventListener('click', () => {
                    ...
                    // Identificar y actualizar los campos dinámicamente
                    Object.keys(fieldMapping).forEach(field => {
